...
export class Player {
  ...
  takeDamage(amount,source){if(this.adminInvulnerable||!this.alive||source&&performance.now()<this.spawnProtectedUntil)return false;this.health=Math.max(0,this.health-amount);if(this.armor>0){this.armor=Math.max(0,this.armor-Math.ceil(amount*.35));}if(this.health===0){this.alive=false;this.deaths++;return true;}this.lastAttacker=source;return false;}
