...
export class BombDefusalMode extends GameMode {
  constructor(game,rounds){super(game);this.rounds=rounds;this.carrier=null;this.planted=false;this.site=null;this.timer=0;this.progress=0;this.defuseProgress=0;this.beep=0;this.pickupCooldown=0;this.bomb=this.createBomb();this.game.scene.add(this.bomb);this.bomb.visible=false;}
  ...
  beginRound(){
    this.carrier=null;this.planted=false;this.site=null;this.timer=this.game.settings.values.bombTime;this.progress=0;this.defuseProgress=0;this.pickupCooldown=0;this.bomb.visible=false;
    const entities=[this.game.player,...this.game.botManager.bots];
    for(const entity of entities){entity.hasBomb=false;entity.bombSite=null;this.setPlayerBomb(entity,false);}
    ...
    if(this.carrier){this.carrier.hasBomb=true;this.carrier.bombSite=this.game.mapConfig.bombSites[Math.floor(Math.random()*this.game.mapConfig.bombSites.length)];this.setPlayerBomb(this.carrier,true);}
    this.emitState();
  }
  update(dt){if(!this.active||this.rounds.state!=='live')return;this.pickupCooldown=Math.max(0,this.pickupCooldown-dt);if(this.carrier&&!this.carrier.alive){this.drop(this.carrier.position);}
  ...
  drop(position){if(this.planted||!this.carrier||!position)return false;const carrier=this.carrier;carrier.hasBomb=false;carrier.bombSite=null;this.setPlayerBomb(carrier,false);this.carrier=null;this.pickupCooldown=.8;this.bomb.visible=true;this.bomb.position.set(position.x,.18,position.z);this.emitState();return true;}
  pickup(entity){if(this.planted||this.carrier||this.pickupCooldown>0||!entity?.alive||entity.team!=='attackers')return false;this.carrier=entity;entity.hasBomb=true;entity.bombSite=this.game.mapConfig.bombSites.reduce((best,site)=>!best||entity.position.distanceToSquared(new THREE.Vector3(site.x,0,site.z))<entity.position.distanceToSquared(new THREE.Vector3(best.x,0,best.z))?site:best,null);this.setPlayerBomb(entity,true);this.bomb.visible=false;this.emitState();return true;}
  setPlayerBomb(entity,owned){if(!entity?.isPlayer)return;if(typeof entity.inventory?.setBomb==='function')entity.inventory.setBomb(owned);else if(entity.inventory?.slots)entity.inventory.slots.bomb=owned;}
