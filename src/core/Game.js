...
import { COMBAT, ECONOMY, hitDamage } from '../config/MatchRules.js';
...
export class Game extends EventTarget {
  ...
  async start({team,mode,onlineSession=null}){this.disposeMatch();this.leaving=false;this.active=true;this.modeId=mode;this.onlineSession=onlineSession;this.mapConfig=MAP_CONFIG;this.scene=new THREE.Scene();this.scene.background=new THREE.Color(0x9fb7bf);this.scene.fog=new THREE.Fog(0xa9b2a4,85,this.settings.values.drawDistance);this.camera=new THREE.PerspectiveCamera(this.settings.values.fov,innerWidth/innerHeight,.08,this.settings.values.drawDistance);this.scene.add(this.camera);this.setupLights();this.collision=new CollisionWorld(MAP_CONFIG);this.navigation=new NavigationGraph(MAP_CONFIG);this.map=new DustInspiredMap(this.scene,MAP_CONFIG,this.settings).build();await this.assets.loadUserConfig();
    this.player=new Player(team,this.skinManager.knife);this.player.name=this.onlineSession?.localName||'Игрок';this.player.spawn(getSpawn(team,0));this.playerMovement=new PlayerMovement(this.player,this.collision,this.input);this.playerCamera=new PlayerCamera(this.camera,this.player,this.input,this.settings);this.weaponManager=new WeaponManager(this.camera,this.player,this.input,this.audio,this.skinManager);this.playerController=new PlayerController(this.player,this.playerMovement,this.playerCamera,this.input,this.weaponManager);this.botManager=new BotManager(this.scene,this.navigation,this.collision,this.audio,this.settings);this.botManager.create(this.onlineSession?0:this.settings.values.botsPerTeam);this.botManager.spawnAll();if(this.onlineSession)this.setupOnlinePlayer();this.impacts=new BulletImpactPool(this.scene);this.blood=new BloodEffect(this.scene);this.smoke=new SmokeEffect(this.scene);this.explosion=new ExplosionEffect(this.scene);this.grenades=new GrenadeSystem(this);this.bindMatchEvents();this.setupMode();this.loop=new GameLoop(dt=>this.fixedUpdate(dt),(dt)=>this.render(dt),1/60,error=>this.handleLoopError(error));this.input.attach();this.input.enabled=false;this.mainMenu.open('game-screen');document.getElementById('game-screen').classList.add('active');document.getElementById('click-to-play').classList.add('visible');this.resize();this.loop.start();this.hud.message(this.onlineSession?`Онлайн-матч против ${this.onlineSession.remoteName}`:mode==='bomb'?'Раунд начался':'Командный бой начался');}
  ...
  setupOnlinePlayer(){
    ...
    this.remotePlayer=new RemotePlayer({name:this.onlineSession.remoteName,team:remoteTeam,scene:this.scene,spawn:getSpawn(remoteTeam,0)});
  ...
  handleOnlineMessage(message){
    ...
    if(message.type==='shot'){
      ...
      if(definition)this.safeEffect('звук онлайн-выстрела',()=>this.audio.shot(.58,definition));
      return;
    }
  ...
  fixedUpdate(dt){if(!this.active||this.paused)return;const beforeHealth=this.player.health;this.playerController.fixedUpdate(dt,this.settings.values,this.audio);this.weaponManager.handleInput(this.playerMovement,this.playerCamera);this.handleGameInput();const objectives=this.objectives();this.botManager.update(dt,this.player,objectives,this.smoke);this.grenades.update(dt);this.bombMode?.update(dt);if(this.modeId==='bomb'){this.rounds.update(dt,[this.player,...this.botManager.bots],this.bombMode.planted);}else this.mode.update(dt);this.syncOnline(dt);if(this.player.health<beforeHealth)this.hud.damage();if(!this.player.alive&&!document.getElementById('death-screen').classList.contains('visible'))this.onPlayerDeath();this.impacts.update(dt);this.blood.update(dt);this.smoke.update(dt);this.explosion.update(dt);this.input.endFrame();}
  ...
  handleGameInput(){if(this.input.action('scoreboard'))this.scoreboard.show([this.player,...this.botManager.bots,...(this.remotePlayer?[this.remotePlayer]:[])]);else this.scoreboard.hide();if(this.input.justPressed('buy'))this.openBuy();if(this.input.justPressed('team')&&!this.onlineSession)this.openMatchMenu('team-menu');if(this.input.justPressed('knifeMenu'))this.openMatchMenu('knife-menu');if(this.input.justPressed('grenades'))this.throwGrenade();if(this.input.justPressed('drop'))this.dropWeapon();}
  ...
  handleShots({weapon,directions}) {
    ...
    for (const direction of directions) {
      ...
      const hit = ray.intersectObjects([...this.map.raycastTargets, ...this.botManager.targets(), ...(this.remotePlayer?.targets()||[])], true)[0];
      if (!hit) continue;
      ...
      if(entity.isRemotePlayer)this.onlineSession?.send({type:'damage',amount,weapon:weapon.definition.name});
  ...
  spawnEntity(entity){if(entity===this.player){this.player.inventory=new PlayerInventory(this.player.team,this.skinManager.knife);this.player.spawn(randomSpawn(this.player.team));this.weaponManager.rebuild();this.stopSpectating();this.sendOnlineState();document.getElementById('click-to-play').classList.add('visible');}else if(!entity.isRemotePlayer)entity.spawn(randomSpawn(entity.team));}
