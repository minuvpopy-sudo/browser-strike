...
export class Game extends EventTarget {
  ...
  async start({team,mode,onlineSession=null,mapConfig=MAP_CONFIG}){this.disposeMatch();this.leaving=false;this.active=true;this.modeId=mode;this.onlineSession=onlineSession;this.mapConfig=mapConfig;this.scene=new THREE.Scene();this.scene.background=new THREE.Color(0x9fb7bf);this.scene.fog=new THREE.Fog(0xa9b2a4,85,this.settings.values.drawDistance);this.camera=new THREE.PerspectiveCamera(this.settings.values.fov,innerWidth/innerHeight,.08,this.settings.values.drawDistance);this.scene.add(this.camera);this.setupLights();this.collision=new CollisionWorld(this.mapConfig);this.navigation=new NavigationGraph(this.mapConfig);this.map=new DustInspiredMap(this.scene,this.mapConfig,this.settings).build();await this.assets.loadUserConfig();
