...
export class Game extends EventTarget {
  ...
  render(dt){if(!this.active)return;if(this.player.alive)this.playerController.visualUpdate(dt);else this.updateSpectatorCamera(dt);this.remotePlayer?.updateVisual(dt);this.updateSunShadow();const targetFov=this.weaponManager.scoped?this.settings.values.fov*.45:this.settings.values.fov;this.camera.fov=THREE.MathUtils.lerp(this.camera.fov,targetFov,Math.min(1,dt*12));this.camera.updateProjectionMatrix();this.hud.update(this,dt);this.updateScoreboard();this.updateBuyContext();this.renderer.render(this.scene,this.camera);}
