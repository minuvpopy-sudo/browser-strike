...
export function effectiveBuyCost(cost, free = false) { return free ? 0 : Math.max(0, Number(cost) || 0); }
export function canBuy({ money, cost, inBuyZone, buyTimeLeft, available = true, free = false }) { return available && inBuyZone && buyTimeLeft > 0 && money >= effectiveBuyCost(cost, free); }
