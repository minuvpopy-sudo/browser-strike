...
export class BotManager extends EventTarget {
  ...
  create(countPerTeam){this.dispose();let index=0;for(const team of ['attackers','defenders'])for(let i=0;i<countPerTeam;i++){const bot=new Bot(`BOT ${NAMES[(index*7+3)%NAMES.length]}`,team,this.scene,index++);bot.freeBuy=Boolean(this.freeBuy);this.bots.push(bot);this.ais.set(bot,new BotAI(bot,this.graph,this.collision,this.settings.values.difficulty,this.audio));}}
  ...
  update(dt,player,objectives,smoke){if(this.frozen){for(const bot of this.bots)bot.velocity?.set(0,0,0);return;}const alive={attackers:[],defenders:[]};if(player.alive)alive[player.team].push(player);for(const bot of this.bots)if(bot.alive)alive[bot.team].push(bot);const sitePhase=Math.floor(performance.now()/12000);for(const bot of this.bots){const enemies=alive[bot.team==='attackers'?'defenders':'attackers'];const candidates=objectives[bot.team]||objectives.default;const site=bot.bombSite||(Array.isArray(candidates)?candidates[(bot.index+sitePhase)%candidates.length]:candidates);bot.objectiveVector??=new THREE.Vector3();bot.objectiveVector.set(site.x,0,site.z);this.ais.get(bot).update(dt,enemies,bot.objectiveVector,smoke,(killer,victim,weapon)=>this.dispatchEvent(new CustomEvent('kill',{detail:{killer,victim,weapon}})));bot.updateVisual(dt);}}
