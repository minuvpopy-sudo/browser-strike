...
export class BotManager extends EventTarget {
  ...
  update(dt,player,objectives,smoke){const alive={attackers:[],defenders:[]};if(player.alive)alive[player.team].push(player);for(const bot of this.bots)if(bot.alive)alive[bot.team].push(bot);const sitePhase=Math.floor(performance.now()/12000);for(const bot of this.bots){const enemies=alive[bot.team==='attackers'?'defenders':'attackers'];const candidates=objectives[bot.team]||objectives.default;const site=bot.bombSite||(Array.isArray(candidates)?candidates[(bot.index+sitePhase)%candidates.length]:candidates);bot.objectiveVector??=new THREE.Vector3();bot.objectiveVector.set(site.x,0,site.z);this.ais.get(bot).update(dt,enemies,bot.objectiveVector,smoke,(killer,victim,weapon)=>this.dispatchEvent(new CustomEvent('kill',{detail:{killer,victim,weapon}})));bot.updateVisual(dt);}}
