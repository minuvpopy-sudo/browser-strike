...
export class BotManager extends EventTarget {
  ...
  spawnAll(){const used={attackers:0,defenders:0};for(const bot of this.bots){const slot=used[bot.team]++;bot.playerTarget=slot===0;const point=this.mapConfig.custom?getSpawn(bot.team,slot+1,this.mapConfig):BOT_FRONTLINE_SPAWNS[bot.team][slot]||getSpawn(bot.team,slot+1,this.mapConfig);bot.spawn(point);this.ais.get(bot)?.reset();}}
