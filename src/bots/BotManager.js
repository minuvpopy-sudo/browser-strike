...
export class BotManager extends EventTarget {
  constructor(scene,graph,collision,audio,settings){super();this.scene=scene;this.graph=graph;this.collision=collision;this.audio=audio;this.settings=settings;this.bots=[];this.ais=new Map();}
  ...
  spawnAll(){const used={attackers:0,defenders:0};for(const bot of this.bots){const slot=used[bot.team]++;bot.playerTarget=slot===0;const point=BOT_FRONTLINE_SPAWNS[bot.team][slot]||getSpawn(bot.team,slot+1);bot.spawn(point);}}
  update(dt,player,objectives,smoke){const all=[player,...this.bots];for(const bot of this.bots){const enemies=all.filter(e=>e.team!==bot.team&&e.alive);const nearest=enemies.reduce((best,enemy)=>!best||bot.position.distanceToSquared(enemy.position)<bot.position.distanceToSquared(best.position)?enemy:best,null);const candidates=objectives[bot.team]||objectives.default;const site=bot.bombSite||(Array.isArray(candidates)?candidates[(bot.index+Math.floor(performance.now()/12000))%candidates.length]:candidates);const objective=nearest&&!bot.hasBomb?nearest.position:site;this.ais.get(bot).update(dt,enemies,new THREE.Vector3(objective.x,0,objective.z),smoke,(killer,victim,weapon)=>this.dispatchEvent(new CustomEvent('kill',{detail:{killer,victim,weapon}})));bot.updateVisual(dt);}}
